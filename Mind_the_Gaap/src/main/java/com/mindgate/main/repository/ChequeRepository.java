...
public class ChequeRepository implements ChequeRepositoryInterface {
	...
	private final static String UPDATE_EXISTING_CHEQUE = "update cheque set ISSUE_CHEQUE_DATE=?,AMOUNT =?,CLEARANCE_CHEQUE_DATE=?,CHEQUE_STATUS=?  where CHEQUE_ID=?";
	...
	public Cheque updateCheque(Cheque cheque) {
		Object[] parameters = {cheque.getChequeDate(), cheque.getAmount(), 
				 cheque.getClearanceChequeDate(), cheque.getChequeStatus(),
				cheque.getChequeId() };
	...
	public boolean addNewCheque(Cheque cheque) {
		...
		Object[] parameters = { cheque.getChequeDate(), cheque.getAmount(), cheque.getFromAccountId(),
                cheque.getToAccountId(), cheque.getClearanceChequeDate(), cheque.getChequeStatus() };
